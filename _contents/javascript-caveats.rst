...
note that if accessed using membership operatory property name is constrained
...
object. But what if the method is overriden accidentally ?
...
handling in Javascript. But the fact is, it is bad programming to use setInterval
a.k.a periodic-timeout if we doubt that the handler function is going to take
more time than the next timeout. One possible solution is given in the above
snippet itself.
...
programmatically. This applies even to global ``this``, as demostrated by the
...
in closures, parent function's namespaces are only refered by enclosed
